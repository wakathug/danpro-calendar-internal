const CALENDAR_CONFIG = Object.freeze({
  ...
  signingSecretProperty: 'INTERNAL_GAS_SIGNING_SECRET',
  accessPolicyHmacSecretProperty: 'ACCESS_POLICY_HMAC_SECRET',
  signatureClockSkewSeconds: 120,
  nonceCacheTtlSeconds: 180,
  nonceCachePrefix: 'signed-request:v1:',
...
/**
 * Vercel Functionからの署名済みPOSTだけを処理します。
...
function doPost(event) {
  try {
    ...
    const accessDenied = isAccessDeniedError_(error);
    return createJsonOutput_({
      ok: false,
      error: accessDenied ? 'ACCESS_DENIED' : 'INVALID_REQUEST',
    });
  }
}

function parseSignedRequest_(event) {
  const raw = event && event.postData && event.postData.contents;
  if (typeof raw !== 'string' || raw.length < 2 || raw.length > 20000) {
  ...
  if (
    !request
    || Object.keys(request).sort().join(',') !== expectedKeys.join(',')
    || request.version !== 1
    || !['authorize', 'calendar', 'dayDetails'].includes(request.action)
    || typeof request.body !== 'object'
    || request.body === null
    || Array.isArray(request.body)
    || !/^[0-9]{10}$/.test(request.timestamp)
    || !/^[A-Za-z0-9_-]{32}$/.test(request.nonce)
    || !/^[A-Za-z0-9_-]{43}$/.test(request.bodyHash)
    || !/^[A-Za-z0-9_-]{43}$/.test(request.signature)
  ) throw new Error('invalid signed request');
  request.email = normalizeEmail_(request.email);
  if (!request.email) throw new Error('invalid signed request');
  return request;
}
...
function getInternalApiCalendarData_() {
  ...
  try {
    cached = parseCachedCalendarAggregate_(
      CacheService.getScriptCache().get(CALENDAR_CONFIG.calendarCacheKey),
    );
  } catch (error) {
    cached = null;
  }
...
function getInternalApiDayDetails_(dateKey, expectedRevision) {
  ...
  if (normalizedRevision && !/^[A-Za-z0-9_-]{1,128}$/.test(normalizedRevision)) {
